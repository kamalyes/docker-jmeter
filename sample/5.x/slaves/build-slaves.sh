docker build --tag kamalyes/jmeter-slave .
# docker network create --driver bridge --subnet 172.19.0.0/16 jmeter
for index in $(seq 1 6)
do   
docker run -it -d --privileged=true --net=jmeter --name jmeter-slave${index} -e JMETER_JMX="test-influxdbv1.jmx" -e JMETER_JVM_ARGS="-Xmx2G -Xms1G" -p 1009${index}:1099 -p 5000${index}:50000 kamalyes/jmeter-slave
